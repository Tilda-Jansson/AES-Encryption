#include <cstdint>
#include <vector>
#include <iostream>
#include <iomanip>
#include <sstream>
#include <cstring>

typedef uint8_t byte;
typedef uint32_t word;

// Precomputed gmul arrays
const byte gmul_by_2[256] = 
{
        
};
const byte gmul_by_3[256] = 
{
        
};

/** Rijndael_S-box
 * copied from : https://en.wikipedia.org/wiki/Rijndael_S-box
 */
const byte s_box[256] = {
    
};

// Rcon array
const word Rcon[11] = {
    0x00000000, // Not used
    0x01000000, // Rcon[1]
    0x02000000, // Rcon[2]
    0x04000000, // Rcon[3]
    0x08000000, // Rcon[4]
    0x10000000, // Rcon[5]
    0x20000000, // Rcon[6]
    0x40000000, // Rcon[7]
    0x80000000, // Rcon[8]
    0x1b000000, // Rcon[9]
    0x36000000  // Rcon[10]
};



word SubWord(word w) {
    return (word(s_box[w >> 24] & 0xff) << 24) |
           (word(s_box[(w >> 16) & 0xff] & 0xff) << 16) |
           (word(s_box[(w >> 8) & 0xff] & 0xff) << 8) |
           (word(s_box[w & 0xff] & 0xff));
}

word RotWord(word w) {
    return (w << 8) | (w >> 24);
}

void KeyExpansion(const byte key[], std::vector<word>& w, int Nk) {
    int Nr = Nk + 6; // Number of rounds
    word temp;
    int i = 0;

    while (i < Nk) {
        w[i] = (word(key[4 * i]) << 24) |
               (word(key[4 * i + 1]) << 16) |
               (word(key[4 * i + 2]) << 8) |
               word(key[4 * i + 3]);
        i++;
    }

    i = Nk;

    while (i < 4 * (Nr + 1)) {
        temp = w[i - 1];
        if (i % Nk == 0) {
            temp = SubWord(RotWord(temp)) ^ Rcon[i / Nk];
        } else if (Nk > 6 && i % Nk == 4) {
            temp = SubWord(temp);
        }
        w[i] = w[i - Nk] ^ temp;
        i++;
    }
}



void SubBytes(byte state[4][4]) {
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            state[i][j] = s_box[state[i][j]];
        }
    }
}


void ShiftRows(byte state[4][4]) {
    byte temp;

    // Shift row 1 (Left shift by 1)
    temp = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = temp;

    // Shift row 2 (Left shift by 2)
    temp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = temp;
    temp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = temp;

    // Shift row 3 (Left shift by 3)
    temp = state[3][0];
    state[3][0] = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = temp;
}


// gmul function for field multiplication NOT USED
byte gmul(byte a, byte b) {
    byte p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        bool hi_bit_set = a & 0x80;
        a <<= 1;
        if (hi_bit_set) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

void MixColumns(byte state[4][4]) {
    byte temp[4];
    for (int i = 0; i < 4; ++i) {
        temp[0] = state[0][i];
        temp[1] = state[1][i];
        temp[2] = state[2][i];
        temp[3] = state[3][i];

        state[0][i] = gmul_by_2[temp[0]] ^ gmul_by_3[temp[1]] ^ temp[2] ^ temp[3];
        state[1][i] = temp[0] ^ gmul_by_2[temp[1]] ^ gmul_by_3[temp[2]] ^ temp[3];
        state[2][i] = temp[0] ^ temp[1] ^ gmul_by_2[temp[2]] ^ gmul_by_3[temp[3]];
        state[3][i] = gmul_by_3[temp[0]] ^ temp[1] ^ temp[2] ^ gmul_by_2[temp[3]];
    }
}


void AddRoundKey(byte state[4][4], const word roundKey[4]) {
    for (int i = 0; i < 4; ++i) {
        word k = roundKey[i];
        for (int j = 0; j < 4; ++j) {
            state[j][i] ^= (k >> (24 - j * 8)) & 0xff;
        }
    }
}


void Cipher(byte in[16], byte out[16], const std::vector<word>& keySchedule) {
    byte state[4][4];
    // Load the input block into the state array
    for (int i = 0; i < 16; ++i) {
        state[i % 4][i / 4] = in[i];
    }

    // Initial AddRoundKey
    AddRoundKey(state, &keySchedule[0]);

    // Rounds
    for (int round = 1; round < 10; ++round) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, &keySchedule[round * 4]);
    }

    // Final round
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, &keySchedule[10 * 4]);

    // Copy the state to the output array
    for (int i = 0; i < 16; ++i) {
        out[i] = state[i % 4][i / 4];
    }
}

int main() {
    // AES-128 requires a 16-byte key
    byte key[16];

    // Read the 128-bit key from stdin
    if (fread(key, 1, 16, stdin) != 16) {
        std::cerr << "Error reading key from stdin" << std::endl;
        return 1;
    }

    // Initialize key schedule and perform key expansion
    std::vector<word> keySchedule(4 * (10 + 1)); // AES-128 has 10 rounds
    KeyExpansion(key, keySchedule, 4); // Nk = 4 for 128-bit key

    byte block[16], encryptedBlock[16];
    while (true) {
        // Read a block of data from stdin
        size_t bytesRead = fread(block, 1, 16, stdin);
        if (bytesRead == 0) {
            break; // No more data to read
        }
        if (bytesRead < 16) {
            // Pad the last block with zeros if it's not a full block
            memset(block + bytesRead, 0, 16 - bytesRead);
        }

        // Encrypt the block and write to stdout
        Cipher(block, encryptedBlock, keySchedule);
        fwrite(encryptedBlock, 1, 16, stdout);
    }

    return 0;
}



